...
function buildManualContext(startedAt) {
  ...
  return {
    ...
    manualExecutionBatches: buildManualExecutionBatches(fields),
    latestRuns,
    displayTopology,
...
function buildManualExecutionBatches(fields) {
  return [
    {
    ...
    },
    {
      ...
      action:
        `Run ${fields.installerPath || "the installer path from the manual context"}, launch the installed app, then uninstall it.`,
      evidence:
...
function renderReport(context, summary) {
  ...
  const noteRows = Object.entries(context.evidenceNoteStarters)
    ...
    .join("\n");
  const batchRows = context.manualExecutionBatches
    .map((batch) => [
      `| ${batch.name}`,
      batch.gateIds.join("<br />"),
      batch.when,
      batch.action,
      `${batch.evidence} |`
    ].join(" | "))
...
| Batch | Gates | When | Action | Evidence |
| --- | --- | --- | --- | --- |
${batchRows}
