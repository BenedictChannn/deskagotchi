...
| Hatch/custom surfaces deferred | README, roadmap, `PanelView`, IPC channel surface, `pnpm run qa:v2:scope` | User-facing generation and custom pet import/export have no panel route, preload API, IPC channel, panel tab, or overlay action; archived local draft helpers remain as package-boundary research code. |
