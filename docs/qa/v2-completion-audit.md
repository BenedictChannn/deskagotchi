...
`corepack pnpm run qa:v2:closeout` when preparing a release branch so the final
...
| --- | --- | --- |
| Lint, typecheck, unit tests | `.qa-runs/2026-05-11T13-53-52Z-check/report.md`, produced by `corepack pnpm run qa:check` | Passed, 59 tests on commit `8b79c55`. |
| Full automated QA gate | `corepack pnpm run qa` | Includes persisted check evidence, launch, drag, overlay, play, lifecycle, renderer, pet assets, item assets, V2 scope, manual-context smoke, manual-preflight smoke, visual page, and manual acceptance page smoke. Release and five-minute idle remain targeted commands because they are slower and environment-sensitive. |
...
| Multi-monitor geometry | `src/main/windowBounds.test.ts` through `corepack pnpm run check` | Pure bounds coverage for right-side, negative-coordinate, stacked-above, stacked-below, largest-intersection, and fallback layouts. |
...
| Manual acceptance form | `docs/qa/v2-manual-acceptance.html`, `docs/qa/v2-manual-acceptance-screenshot.png`, `corepack pnpm run qa:v2:manual-page` | Ready for remaining physical/manual signoff and JSON export; browser smoke confirms gate counts, required run context blocking, generated session context preload, stale-context reset behavior, pass/defer mutual exclusion, deferral approver blocking, export JSON, and screenshot rendering. |
...
| Offline progression, low-maintenance mode, deadlines, action effects, growth, and storage have tests | `corepack pnpm run check` | None known for current scope. |
| Hatch/custom generation deferred | README, roadmap, `PanelView`, IPC channel surface, `corepack pnpm run qa:v2:scope` | User-facing generation has no panel route, preload API, IPC channel, panel tab, or overlay action; archived local draft helpers remain as package-boundary research code. |
...
closeout report and `corepack pnpm run qa:v2:manual-preflight` list every unresolved
