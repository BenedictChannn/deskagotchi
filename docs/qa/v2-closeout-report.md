...
| Multi-monitor bounds logic covers right, left, stacked, largest-intersection, and fallback layouts. | PASS | src/main/windowBounds.test.ts through corepack pnpm run check |  |
...
| Care simulation is deterministic, documented, and tested for V2 rules. | PASS | simulation docs and test suite through corepack pnpm run check |  |
| Package validation and persistence safety remain covered by the checked test suite. | PASS | package, runtime, and storage tests through corepack pnpm run check |  |
...
```powershell
corepack pnpm run qa:v2:audit --manual C:\path\to\v2-manual-acceptance-export.json --report .qa-runs\v2-closeout-report.md
```
